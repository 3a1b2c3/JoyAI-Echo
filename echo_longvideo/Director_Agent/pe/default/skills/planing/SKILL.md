...
Use only profile-bearing `memory_assets` returned by `get_workplace_status` to
call `set_shot_memory_recommendations`. Match `reference_type`,
`reference_label`, `identity_ids`, and `profile_text` to the target shot instead
of treating every image as a character. This output is a draft for human review,
not approval. Interactive generation consumes only explicitly applied
`approved_memory_slots`; never append `reference_shot_ids` as Memory slots.

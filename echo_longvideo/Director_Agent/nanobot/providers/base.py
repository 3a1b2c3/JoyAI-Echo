...
class _LLMRequestWindowStats:
    """Rolling 60-second LLM request counter for rate-limit diagnostics."""
...
_llm_request_window_stats = _LLMRequestWindowStats()
...
class LLMProvider(ABC):
    ...
    def _is_rate_limit_response(cls, response: LLMResponse) -> bool:
        """Return True when the provider response indicates throughput/rate limiting."""
        ...
        if semantic_tokens.intersection(cls._RETRYABLE_429_ERROR_TOKENS):
            return True

        content = (response.content or "").lower()
        return any(marker in content for marker in cls._RATE_LIMIT_ERROR_MARKERS)

    @classmethod
    def _log_rate_limit_stats(
        cls,
        ...
        *,
        ...
        requests_last_60s = _llm_request_window_stats.count_last_minute()
        snippet = (response.content or "")[:120].lower()
        if giving_up:
            logger.warning(
                "LLM rate limit after {} attempts, giving up: requests_last_60s={} model={} error={!r}",
                attempt,
                requests_last_60s,
                model or "-",
                snippet,
            ...
            return
        logger.warning(
            "LLM rate limit (attempt {}, retrying in {}s): requests_last_60s={} model={} error={!r}",
            attempt,
    ...
    async def _run_with_retry(
        ...
        *,
        ...
        while True:
            attempt += 1
            _llm_request_window_stats.record()
            ...
            if not self._is_transient_response(response):
                ...
                if stripped is not None and stripped != kw["messages"]:
                    logger.warning(
                        ...
                        "error_status={} error_kind={} error_type={} error_code={} content={!r}",

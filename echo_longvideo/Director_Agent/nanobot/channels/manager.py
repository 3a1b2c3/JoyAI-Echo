...
class ChannelManager:
    ...
    def _configured_memory_review_runner(self):
        """Bind Memory VLM calls to the same configured model gateway."""
        ...
        review = self.config.tools.memory_review
        model = review.model or self.config.agents.defaults.model
        provider = review.provider or None
        return partial(
            run_memory_review_from_config,
            api_base=self.config.get_api_base(model, provider=provider),
            api_key=self.config.get_api_key(model, provider=provider) or "",
            vlm_model=model,
        ...
        for name, cls in discover_all().items():
            ...
            try:
                ...
                if cls.name == "websocket" and self._session_manager is not None:
                    ...
                    kwargs["model"] = self.config.agents.defaults.model
                    kwargs["tools_config"] = self.config.tools
                    kwargs["gateway_debug"] = self._gateway_debug
                    kwargs["memory_review_runner"] = (
                        self._configured_memory_review_runner()
                    )
                    static_path = _default_webui_dist()
                    if static_path is not None:
                        kwargs["static_dist_path"] = static_path
                if cls.name == "director_callback" and self._session_manager is not None:
                    kwargs["workspace"] = self._session_manager.workspace
                    kwargs["tools_config"] = self.config.tools
                    kwargs["memory_review_runner"] = (
                        self._configured_memory_review_runner()
                    )

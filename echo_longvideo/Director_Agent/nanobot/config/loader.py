...
def _migrate_config(data: dict) -> dict:
    ...
    # Move the legacy Memory VLM model out of the video-service section.
    echo = (
        tools.get("echoGenerator")
        or tools.get("echo_generator")
        or tools.get("directorRemote")
        or tools.get("director_remote")
        or {}
    )
    memory_review = tools.get("memoryReview") or tools.get("memory_review")
    if memory_review is None:
        memory_review = tools.setdefault("memoryReview", {})
    legacy_vlm_model = echo.pop("vlmModel", None) or echo.pop("vlm_model", None)
    if legacy_vlm_model and "model" not in memory_review:
        memory_review["model"] = legacy_vlm_model

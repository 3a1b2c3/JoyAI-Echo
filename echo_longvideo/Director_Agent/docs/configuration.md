...
Release config: `.config.local.json`

> [!NOTE]
> Copy `.config.local.example.json` to `.config.local.json`, then pass it with `--config` when running
> the gateway. The public template enables only the WebUI WebSocket and Director callback channels.
...
{
  "providers": {
    "custom": { "apiKey": "${NANOBOT_MODEL_API_KEY}" }
  },
  "tools": {
    "fileStorage": {
      "outbound": {
...
NANOBOT_MODEL_API_KEY=your-key-here
FILE_STORAGE_ACCESS_KEY_ID=optional-storage-key
FILE_STORAGE_SECRET_ACCESS_KEY=optional-storage-secret
```

## Echo Director

Keep generic agent settings under `agents.defaults` and Echo service connectivity under
`tools.echoGenerator`:
...
`maxConcurrentRequests` limits simultaneous agent turns; set it to `0` for no limit.
Generation and merge jobs require the Echo service and complete asynchronously through callbacks.
...
nanobot agent -c .config.local.json -m "Hello!"

# One-off workspace override on top of that config
nanobot agent -c .config.local.json -w /tmp/echo-director-test -m "Hello!"
```
...
The personal release uses the WebUI WebSocket channel and the Director callback channel. Configure
both under `channels` in `.config.local.json`:
...
Echo jobs use callback-only completion. Point the Echo client back to the local callback channel:
...
Keep `callbackBaseUrl` aligned with the `director_callback` host and port. `GET /version/{id}` remains
available on Echo Server for diagnostics, but the Agent does not run a status-polling loop.
...
By default, each channel and chat ID combination gets its own session. To share one conversation
across multiple configured transports, enable `unifiedSession`:
...
When enabled, all incoming messages are routed into a single shared session.
...
| Existing `session_key_override` | Respected | Still respected — not overwritten |

...
| `websocketRequiresToken` | bool | `false` | When `true` and no static `token` is configured, clients must still present a valid issued token. Set to `false` to allow unauthenticated connections (only safe for local/trusted networks). |
...
- **Local-first default**: `websocketRequiresToken` defaults to `false`. Enable it whenever the service is reachable beyond a trusted local environment.

...
function titleFor(s: ChatSummary, fallbackTitle: string): string {
  const title = deriveTitle(s.preview, fallbackTitle);
  return title.length > 48 ? `${title.slice(0, 45)}…` : title;
}

function sessionModeLabelKey(s: ChatSummary): string {
  if (s.autoGenerate === true) return "chat.mode.autoGenerate";
  return "chat.mode.stepwise";
}
          ...
          return (
            <li key={s.key}>
              ...
              >
                ...
                >
                  <span className="w-full truncate font-medium leading-5 break-all whitespace-break-spaces">
                    {title}
                  </span>
                  <span className="flex items-center gap-1.5 text-[10.5px] text-muted-foreground/80">
                    <span className="rounded bg-foreground/8 px-1 py-px text-[9.5px] font-medium text-muted-foreground">
                      {t(sessionModeLabelKey(s))}
                    </span>
